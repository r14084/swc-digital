...
def main() -> None:
    ...
    for disp, src_prefix, group in CHOSEN:
        ...
        # Insert PROGMEM before the initializer and rename splash_ -> mascot_.
        def progmem(s: str) -> str:
            s = s.replace(src_prefix, dst_prefix)
            return re.sub(r"\] = \{", "] PROGMEM = {", s, count=1)

        blocks.append(progmem(palette))
        blocks.append(progmem(frames))
        blocks.append(progmem(holds))
        blocks.append("")
...
//
// Curated subset of the claudepix 20x20 pixel-art creature animations, re-emitted
// PROGMEM for the ESP8266 (frame data stays in flash, not the ~40 KB heap).
...
typedef struct {{
    ...
    const uint16_t *palette;      // PROGMEM, MASCOT_PALETTE_SIZE entries
    const uint8_t (*frames)[400]; // PROGMEM, frame_count x 400 cells
    const uint16_t *holds;        // PROGMEM, frame_count hold times (ms)
}} MascotAnim;
